...
1. **Regional risk heatmap** (Jul–Sep tropical-cyclone window), shaded with a green data ramp from light (`#e8f1de`) to very dark (`#16290d`). States are grouped into named regions (FLA, SE, TEXAS, MID-ATL, NEW ENGLAND, …) and each region is colored by the *average* score of its member states. Each region carries a single rounded white callout label centered on it. States not in any region render in grayscale neutral.
2. **Live NHC storm cones.** The page polls NOAA's National Hurricane Center tropical REST service every 5 minutes and outlines active forecast cones in the accent color `#ec7700`. The NHC feed covers the Atlantic, Gulf, and Caribbean basins.
3. **Reefer lane alerts.** A small editable set of trucking lanes is rendered as polylines with a white callout box at each lane's origin. Each lane is tested against every active cone polygon using turf.js (`booleanIntersects`); any lane that crosses a cone is flagged in `#ec7700`.
...
| Seasonal outlook       | **NOAA CPC** Atlantic Hurricane Season Outlook ([source](https://www.cpc.ncep.noaa.gov/products/outlooks/hurricane.shtml)) | Pinned by hand in `SEASONAL_OUTLOOK` (no clean REST endpoint). CPC issues two outlooks per season — initial in late May, update in early August. Refresh procedure documented inline in `index.html`. |
